...
var utils = {
    ...
    extract_object: function(obj, field) {
...
var openstreetmaps_api = {
    ...
    get_addresses: function(user_reply, http) {
        var addresses;
        return http
    ...
    // Removes the display_name property if it was not marked for
    // being saved against the contact in opts.map_api_opts.store_fields
    cleanup_store_field: function(data) {
        if (openstreetmaps_api.store_fields.indexOf('display_name') === -1) {
...
var googlemaps_api = {
    ...
    extract_addresses: function(results) {
        ...
        results.forEach(function(res) {
            var result = {};        // Used to generate single result object
            googlemaps_api.store_fields.forEach(function(field) {
                result[field] = utils.extract_object(res, field);
            });
        ...
        return http
            ...
            .then(function(resp) {
                if(!resp.data || resp.data.status != 'OK') {
                    ...
                    addresses = googlemaps_api
                                    .extract_addresses(resp.data.results);
                }
                return addresses;
            });
    },
    ...
    // Removes the formatted_address property if it was not marked for
    // being saved against the contact in opts.map_api_opts.store_fields
    cleanup_store_field: function(data) {
        if (googlemaps_api.store_fields.indexOf('formatted_address') === -1) {
...
var LocationState = State.extend(function (self, name, opts) {
    ...
    */
    ...
    if (opts.mapping_service === 'gmaps') {
        map_api = googlemaps_api;
    } else if (opts.mapping_service === 'osmaps') {
        map_api = openstreetmaps_api;
    } else {
        map_api = opts.mapping_service;
    }

    // Override map_api defaults
    Object.keys(opts.map_api_opts).forEach(function(key) {
        map_api[key] = opts.map_api_opts[key];
    });
    ...
    // Depending on API response, select next state
    self.handlers.initial = function(content) {
        return map_api
            .get_addresses(content, self.http)
            .then(function(addresses) {
                ...
                // If only one result is found, store and enter next state
                    ...
                    return self
                        .store_contact_data(addresses[0])
                        .then(function() {
                            return self.set_next_state(self.next);
                        });
                }
    ...
    self.generate_pages = function(addresses) {
        ...
        while(current_address_number < addresses.length) {
            ...
            while(current_option_number <= self.options_per_page) {
                var choice_text = map_api.format_choice(current_option_number,
    ...
    self.generate_skip_footer = function() {
        var skip_footer;
        if (self.skip !== null) {
            skip_footer = '\n' + 's. ' + self.skip_text;
        } else {
    ...
    self.store_contact_data = function(data) {
        ...
        map_api.cleanup_store_field(data);

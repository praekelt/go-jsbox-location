    ...
    */
    ...
    self.handlers.refinequestion = function(content) {
        var handler = {
            ...
            s: self.handlers.skip,
            r: self.handlers.initial
        }[content];
    ...
    // User selects Skip
    self.handlers.skip = function(content) {
        return self.set_next_state(self.next, null);
    };
    ...
    self.generate_footer = function(content) {
        var options = [];
        if (self.metadata.current_page >= self.metadata.pages.length && self.retry_text){
            options.push('r. ' + self.retry_text);}
        else {options.push('n. ' + self.next_text);}
        options.push('p. ' + self.previous_text);
        if (self.skip_text) {

...
var LocationState = State.extend(function (self, name, opts) {
    ...
    :param array opts.address_details:
        Optional list for OpenStreetMaps that defines which address details will
        be displayed back to the user to select their address from.  Defaults
        to ``['house_number', 'road', 'suburb', 'city', 'state', 'postcode',
        'country']``
    ...
    */
    ...
    opts = _.defaults(opts || {}, {
        ...
        hard_boundary:true,
        address_limit:30,
        address_details:['house_number', 'road', 'suburb', 'city', 'state',
                        'postcode', 'country']
    });
    ...
    self.on('state:input', function(event) {  // george : maybe we can use for mmc?
    ...
    self.extract_from_osmaps = function(results) {
        ...
        for (var result in results) {
            ...
            for (var detail in self.address_details) {
                var address_detail = results[result].address[self.address_details[detail]];
                if (address_detail !== undefined) {
                    var address_string;
                    if (address_obj.formatted_address === '') {
                        address_string = address_detail;
                    } else {
                        address_string = ', ' + address_detail;
                    }
                    address_obj.formatted_address = (
                        address_obj.formatted_address + address_string);
                }
            }
    ...
    // george: i guess the question here is if we should have a default naming system that we save data into that
    // is the same for gmaps and osmaps
    ...
    self.get_viewbox = function() {
        ...
        return "viewbox=" + self.bounding_box[0] + "%2C" + self.bounding_box[1] +
               "%2C" + self.bounding_box[2] + "%2C" + self.bounding_box[3];
    };
